...
app.post("/api/rory-gpkos/compile", (req, res) => {
  ...
  try {
    ...
    if (language.toLowerCase() === 'cpp') {
       sandboxOutput.push(`📦 Loading GCC headers (iostream, vector, string, algorithm)...`);
    } else if (language.toLowerCase() === 'java') {
       sandboxOutput.push(`📦 Resolving package dependencies and java.util.*...`);
    } else if (language.toLowerCase() === 'python') {
       sandboxOutput.push(`📦 Initializing pip modules...`);
    }
    ...
    // Check if user tries to include standard headers correctly
    if (language.toLowerCase() === 'cpp' && code.includes('#include')) {
       sandboxOutput.push(`[SYSTEM_MSG] All C++ standard libraries linked correctly.`);
       if (!foundOutput) {
          sandboxOutput.push(`[STDOUT] Hello, World! (Simulated output due to complex stream operations)`);
          foundOutput = true;
       }
    }
